...
# miniBox lives in the miniHawk checkout as a submodule; the guest toolchain (musl + libstdc++ for
# the sandbox) is part of its meson graph.
...
ppsspp_pin="$(git -C "$root/extern/ppsspp" describe --tags --always 2>/dev/null || echo unknown)"
...
# the frontend extracts packages into build/CoreCache keyed by content; drop any stale extraction
# so the next load picks this build up
for cache in "$minihawk_root"/build/CoreCache/ppsspp-*; do
